...
bool hardware_timer_init(void) {
  ...
  if (HAL_TIM_RegisterCallback(&HTIM, HAL_TIM_BASE_MSPINIT_CB_ID,
                               timer_msp_init_cb) != HAL_OK)
    return false;

  if (HAL_TIM_RegisterCallback(&HTIM, HAL_TIM_BASE_MSPDEINIT_CB_ID,
                               timer_msp_deinit_cb) != HAL_OK)
    return false;
...
void TIM_IRQHandler(void) { HAL_TIM_IRQHandler(&HTIM); }
...
bool hardware_timer_init(void) {
  ...
  if (HAL_TIM_RegisterCallback(&htim4, HAL_TIM_BASE_MSPINIT_CB_ID,
                               timer_msp_init_cb) != HAL_OK)
    return false;

  if (HAL_TIM_RegisterCallback(&htim4, HAL_TIM_BASE_MSPDEINIT_CB_ID,
                               timer_msp_deinit_cb) != HAL_OK)
    return false;
  ...
  if (HAL_TIM_RegisterCallback(&htim4, HAL_TIM_PERIOD_ELAPSED_CB_ID,
                               hardware_timer_cb) != HAL_OK)
    return false;
...
void TIM4_DAC_IRQHandler(void) { HAL_TIM_IRQHandler(&htim4); }
...
bool hardware_timer_init(void) {
  ...
  if (HAL_TIM_RegisterCallback(&htim7, HAL_TIM_BASE_MSPINIT_CB_ID,
                               timer_msp_init_cb) != HAL_OK)
    return false;

  if (HAL_TIM_RegisterCallback(&htim7, HAL_TIM_BASE_MSPDEINIT_CB_ID,
                               timer_msp_deinit_cb) != HAL_OK)
    return false;
...
void TIM7_DAC_IRQHandler(void) { HAL_TIM_IRQHandler(&htim7); }
...
#define MAX_NUM_SYMBOL_QUEUE_STR 30
  ...
  instance->timer =
      dispatch_source_create(DISPATCH_SOURCE_TYPE_TIMER, 0, 0, instance->queue);
...
uint32_t rtimer_get_elapsed_time(rtimer *instance) {
  ...
  return (uint32_t)((current_time.tv_sec - instance->last_sig.tv_sec) *
                        1000000 +
                    current_time.tv_usec - instance->last_sig.tv_usec);
}
