...
#if defined(STM32F072xB) || defined(STM32F091xC) || defined(STM32F103xB) || defined(STM32F407xx) || defined(STM32F429xx) || defined(STM32F103xE)
...
bool hardware_timer_init(void)
{
    ...
    if (HAL_TIM_RegisterCallback(&HTIM, HAL_TIM_BASE_MSPINIT_CB_ID, timer_msp_init_cb) != HAL_OK)
        return false;

    if (HAL_TIM_RegisterCallback(&HTIM, HAL_TIM_BASE_MSPDEINIT_CB_ID, timer_msp_deinit_cb) != HAL_OK)
        return false;
...
uint32_t rtimer_get_elapsed_time(rtimer* instance)
{
    ...
    return (current_time.tv_sec - instance->last_sig.tv_sec) * 1000000 + current_time.tv_usec -
